...
class legalProcedure extends connection {

    private $table = 'legal_procedures';
    private $token;
    ...
    public function post($data, $files) {
        ...
        if ($idLegal) {
            $ruta = $this->processFile($idEmpresa, $idLegal, $files);
            if ($ruta) {
                parent::nonQuery("UPDATE {$this->table} SET rutaArchivo='$ruta' WHERE idLegal=$idLegal");
            }
            $resp = $_answers->response;
            $resp['result'] = ['idLegal' => $idLegal, 'rutaArchivo' => $ruta];
            return $resp;
        }
    ...
    public function put($data, $files) {
        ...
        if ($updates) {
            parent::nonQuery("UPDATE {$this->table} SET " . implode(',', $updates) . " WHERE idLegal=$idLegal AND idEmpresa=$idEmpresa");
        }
    ...
    public function delete($data) {
        ...
        $rows = parent::getData("SELECT rutaArchivo FROM {$this->table} WHERE idLegal=$idLegal AND idEmpresa=$idEmpresa");
        parent::nonQuery("DELETE FROM {$this->table} WHERE idLegal=$idLegal AND idEmpresa=$idEmpresa");
    ...
    private function processFile($idEmpresa, $idLegal, $files) {
        if (!isset($files['archivo']) || $files['archivo']['error'] !== UPLOAD_ERR_OK) return null;

        $file      = $files['archivo'];
        $ext       = strtolower(pathinfo($file['name'], PATHINFO_EXTENSION));
        $filename  = "legalProced_{$idLegal}_" . time() . ".$ext";
        $targetDir = "dataClients/{$idEmpresa}/legalProced/";
