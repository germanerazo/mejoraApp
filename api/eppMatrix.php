...
require_once '../class/eppMatrix.class.php';

$_answers = new answers;
$_eppMatrix = new eppMatrix;
...
switch ($_SERVER['REQUEST_METHOD']) {
    case 'GET':
        ...
        if (isset($_GET['idEmpresa'])) {
            $data = $_eppMatrix->getMatrix($_GET['idEmpresa']);
    ...
    case 'POST':
        ...
        if (isset($data['idEmpresa'])) {
            $dataArray = $_eppMatrix->saveMatrix($data);
    ...
    case 'DELETE':
        ...
        if ($id) {
            $dataArray = $_eppMatrix->deleteMatrix($id);
            if (isset($dataArray["result"]["error_id"])) {
                http_response_code($dataArray["result"]["error_id"]);
            } else {
                http_response_code(200);
