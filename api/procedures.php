...
require_once '../class/procedures.class.php';
...
switch ($_SERVER['REQUEST_METHOD']) {
    ...
    case 'GET':
        ...
        if (isset($_GET['idEmpresa'])) {
            $data = $_procedures->listProcedures($_GET['idEmpresa']);
    ...
    case 'POST':
        ...
        $dataArray = $_procedures->post($data, $files);
    ...
    case 'DELETE':
        header('Content-Type: application/json');
        $postBody = file_get_contents('php://input');
        $dataArray = $_procedures->delete(json_decode($postBody, true));
        if(isset($dataArray["result"]["error_id"])) {
            $responseCode = $dataArray["result"]["error_id"];
            http_response_code($responseCode);
        } else {
