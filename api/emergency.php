...
require_once '../class/emergency.class.php';
...
switch ($_SERVER['REQUEST_METHOD']) {
    case 'GET':
        ...
        if (isset($_GET['idEmpresa'])) {
            $data = $_emergency->getEmergency($_GET['idEmpresa']);
    ...
    case 'POST':
        ...
        if (isset($_POST['idEmpresa'])) {
            $dataArray = $_emergency->saveEmergency($_POST, $file);
    ...
    case 'DELETE':
        ...
        if ($id) {
            $dataArray = $_emergency->deleteEmergency($id);
            if (isset($dataArray["result"]["error_id"])) {
                http_response_code($dataArray["result"]["error_id"]);
            } else {
                http_response_code(200);
