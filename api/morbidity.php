...
require_once '../class/morbidity.class.php';

$_answers = new answers;
$_morbidity = new morbidity;
...
switch ($_SERVER['REQUEST_METHOD']) {
    case 'GET':
        ...
        if (isset($_GET['idEmpresa'])) {
            $data = $_morbidity->getMorbidity($_GET['idEmpresa']);
    ...
    case 'POST':
        ...
        if (isset($data['idEmpresa'])) {
            $dataArray = $_morbidity->saveMorbidity($data);
    ...
    case 'DELETE':
        ...
        if ($id) {
            $dataArray = $_morbidity->deleteMorbidity($id);
            if (isset($dataArray["result"]["error_id"])) {
                http_response_code($dataArray["result"]["error_id"]);
            } else {
                http_response_code(200);
